...
<script type="text/javascript">
	...
	$('#fetch-user-cvs').click(function() {
		...
		fetchUserCVs(1, periodFrom, periodTo);
	});
		...
		$.ajax({
            ...
            success: function(success) {
                ...
                for (let cv in cvs) {
                	...
                	var middleName = cvs[cv]['middle_name'];
                	if (middleName == null) {
                		middleName = '';
                	}
                	...
                	for (let i = 0; i < cvs[cv]['techologies'].length; i++) {
                		nameOfTechologies = nameOfTechologies.concat(cvs[cv]['techologies'][i]['name']);
                		if (i + 1 != cvs[cv]['techologies'].length) {
                			nameOfTechologies = nameOfTechologies.concat(', ');
                		}
                	}

                	let tableRow = '<tr class="text-center"> <td>'+ cvs[cv]['first_name'] + ' ' + middleName + ' ' + cvs[cv]['last_name'] +'</td> <td>'+ birtDate +'</td> <td>'+ cvs[cv]['university']['name'] +'</td> <td>'+ nameOfTechologies +'</td> </tr>';
